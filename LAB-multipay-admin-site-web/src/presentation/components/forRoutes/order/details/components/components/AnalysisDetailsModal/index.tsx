...
export const AnalysisDetailsModal: React.FC<AnalysisDetailsModalProps> = ({
  ...
  const isAprovado = payment.status.toLowerCase() === 'aprovado'
  const isRecusado = payment.status.toLowerCase() === 'recusado'
  ...
  const approvalDate = payment.approvedAt ? isoToLocalePtBr(payment.approvedAt) : '00/00/0000 00:00:00'
